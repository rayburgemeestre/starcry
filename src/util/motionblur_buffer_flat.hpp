...
public:
  motionblur_buffer_flat() = default;

  motionblur_buffer_flat(size_t width, size_t height) {
    resize(width, height);
  }
  ...
  void draw_callback(std::function<void(int x, int y, const data::color&)> callback) {
    for (const auto& [x, y] : active_pixels()) {
      // Get the color data and blend it
      const auto& color_dat = get_data(x, y);
