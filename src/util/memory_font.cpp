...
void memory_font::initialize(unsigned char *font_data, size_t font_data_len, const char *font_filename) {
  ...
  allegro_file_ = std::unique_ptr<ALLEGRO_FILE, decltype(&al_fclose)>(
      al_open_memfile(static_cast<void *>(font_data), font_data_len * sizeof(char), "r"), [](ALLEGRO_FILE *) -> bool {
        // al_fclose() is already called by the font
        //  taking the ownership..
        return true;
      });

  allegro_font_ = std::unique_ptr<ALLEGRO_FONT, decltype(&al_destroy_font)>(
      al_load_ttf_font_f(allegro_file_.get(), font_filename, size_, flags_), &al_destroy_font);
}
