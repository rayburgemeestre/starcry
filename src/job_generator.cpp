...
behavior job_generator(stateful_actor<job_generator_data> *self,
  ...
  return {
      [=](initialize) -> message {  // by main
        //        self->state.bitrate = context->run<double>("typeof bitrate != 'undefined' ? bitrate : (500 * 1024 *
        //        8)"); self->state.use_stdin = context->run<bool>("typeof stdin != 'undefined' ? stdin : false");
