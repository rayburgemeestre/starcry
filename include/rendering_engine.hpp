...
public:
    ...
    for (auto shape : shapes) {
      if (shape.type == data::shape_type::circle)
        draw_logic_.render_circle<double>(
            shape.x, shape.y, shape.radius, shape.radius_size, shape.gradient_, shape.blending_);
      else if (shape.type == data::shape_type::line)
        draw_logic_.render_line<double>(
            shape.x, shape.y, shape.x2, shape.y2, shape.radius_size, shape.gradient_, shape.blending_);
  ...
  std::unique_ptr<memory_font> font;
